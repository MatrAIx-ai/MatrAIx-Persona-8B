...
Add a Harbor persona agent named `persona-uxagent`. Port the minimum UXAgent reasoning core into a conversational adapter and replace browser-specific observation/action handling with a VoiceLab API client.

The integration will:

- retain UXAgent's fast loop (`perceive → plan → act`) and slow loop (`reflect → wonder → memory update`);
- change the only executable action to `send_message`;
...
A minimal adaptation of UXAgent's reasoning behavior.
...
Only the UXAgent source and prompts required for the conversational policy are ported. The repository must include:

- UXAgent's MIT license notice for the copied/adapted material;
- upstream URL `https://github.com/neuhai/UXAgent`;
- the exact upstream commit used;
- a short mapping of upstream files to adapted files.
...
- Driver utterances are generated by the adapted UXAgent dual-system policy, not the old user simulator.
